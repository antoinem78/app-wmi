...
**Items 3 to 8 PARKED by the founder on 2026-09-03 as minor.** Items 1 and 2 are done. Item 3 (Shallowford eConnector) belongs to that channel regardless.

1. **Meta token travelled in the URL: DONE 10:15 UTC.** The founder created the Header Auth credential `Meta system-user token (header)` (`5cOSHv4rFTASt7iF`); all 21 Graph nodes across 8 workflows now use it, each PUT read back with zero query-auth nodes left. Proven live: a BERNARD_optimise probe read 159 change-history rows through the header credential and the stored `paging.next` carries no token. Side finding fixed on the way: BERNARD_build had two pairs of nodes sharing a node id (Build/Write counterfactual duplicated Build/Write move snapshots), which n8n's validator now refuses; fresh ids assigned. The generator (`build-optimise-workflows.py`) now emits header auth and the error handler, so a redeploy cannot regress either. **Pruned 10:40 UTC on the founder's word.** Scanned all 153 stored executions of the eight workflows that ever used query auth for a token-shaped string; only BERNARD_optimise carried it, in 8 executions (18 to 26 August), all deleted. A re-scan of the 1,500 most recent executions across every workflow finds none left. The 26 August executions live on only as stripped fixtures in `tests/fixtures/optimise-glue.json`.
2. **RCV_form_routing: DONE.** Founder confirmed it dead (RCV_form_routing_DEV is the live one); deactivated 10:50 UTC. The two HTTP nodes still carry a hardcoded GHL private integration token (`Bearer pit-9…aab3`) in a now-inactive workflow and in the daily GitHub backups. If that token is still valid for its location, rotating it in GHL is the only way to void those copies.
