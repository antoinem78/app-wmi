...
**PROPOSED by Chat 2026-09-03, awaiting the founder's §0 line:** (a) §4, optimise rulings: the 50-move zero-reversal bar counts only moves the verifier recorded unaided; the four hand-reconciled moves of 26 August stay in the record labelled as such and count only if the founder rules so. (b) §9, doctrine: no verifier's count is trusted until a glue-level replay harness has proven it recording real writes unaided; a control never observed catching or recording a real event is declared, not enforced (third instance of the pattern: parser scoring, contract v2 report-shape claim, optimise verifier).
...
**Two proposed edits (§4 optimise bar, §9 doctrine):** recorded above as PROPOSED, awaiting the founder's one word each.
