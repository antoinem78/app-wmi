...
**6.6 HubSpot client (NEW, template to complete).** Fill on founder input:
- **Client, entity, terms:** ?
- **What it must prove:** the strategic question first: **HubSpot as CRM substrate versus HubSpot as an integration target.** The entire substrate is GHL-centric (sub-accounts, PITs, native attribution fields, workflows). A HubSpot client either (a) keeps HubSpot and we integrate (OCT, attribution, agent handoffs into HubSpot objects, reporting from HubSpot data), which makes this the **CRM-agnostic operator blueprint**, or (b) migrates to GHL, which makes it an ordinary Growth client with a migration. The blueprint value is almost entirely in (a). Decide which before building.
- **What it templates if (a):** the HubSpot connector pattern (which objects, which properties, which webhooks), attribution mapping onto HubSpot's own source fields, the handoff-to-task pattern in HubSpot, and the reporting join. This is the pattern every future HubSpot client reuses.
- **Provisioning target:** config and content only, no workflow edits, same bar as GHL clients; where HubSpot forces per-client workflow objects, record them as the same class of exception as the per-client GHL caller.
- **Learnings:** open.
- **Owner of the first write-up:** Code (technical shape) then Chat (blueprint canon), per the provenance rule.
...
| Entity portals (wmiltd GBP, FZCO USD/AED) | LIVE; both proven. **[Code 2026-09-03: "wmiltd funnel unrehearsed" is false. The wmiltd funnel ran end to end on 2026-08-06 with a £1 live Stripe checkout (test client WE BET, still to be deleted and refunded); the portal DB holds 4 paid rows.]** Upsells (one-off and recurring, signable quote for recurring) built 2026-08-11 on both portals, never yet used on a live client | platform architecture, R14 |
...
| **Agent optimise (Bernard, supervised by Norbert)** | **[Code 2026-09-03: missing line.]** LIVE since 2026-08-18, v1.1 exclusions 2026-08-26; 4 moves executed, 3 rejected, graduation 4 of 50 | `BERNARD_OPTIMISE_SPEC.md`, design note, `AGENT_OPTIMISE_PROCESS_FEEDBACK.md` |
...
**Founder:** R17, R18 (a/b/c), E1, E2; **[Code 2026-09-03 additions:** incident-mode ruling; defensive-moves-exempt-from-reversal ruling; publish the six DM nurture drafts (UI-only, checklist in `docs/clients/dental-mastery.md`); Cloudflare login + Google OAuth to deploy the booking engine; delete WE BET and refund the £1; run `0025` on the wmiltd SQL editor is done; VIP nurture UI build.**]** Kelsey Lane identity; Shopify tokens (instawarm, Brunos); SLACK_META_REVIEW_CHANNEL env; two quarantined AB ads; wmiltd portal readiness before Super Henry; skincare V1-V8; Joey engagement running; HubSpot client details for §6.6; upload amended Offers_and_Pricing to Drive (or leave to revival checklist).
**Code:** **[Code 2026-09-03: the 5 Aug risk brief §1 is CLOSED, all five items, 2026-08-12 to 2026-08-18: `MIGRATIONS.md` + `scripts/migrate.mjs`, `META_PORTFOLIO_RISK_NOTE.md` (premise was wrong: seven client-owned portfolios, the single point is our token), `EXECUTOR_CONTRACT_CONTROL_AUDIT.md` + 29 parser tests, `CLAIMS_GATE_DESIGN_NOTE.md`, `ECONOMICS_LEDGER.md`. Denis brief: items 1, 2, 3, 4 done; 5a dupe-exclusion audience not done. The four-item agent delta is §8 above.]** Still open: Oscar's Norbert insertion (spec §9); Shallowford lessons digest (other machine, folded into its channel-file rewrite); DM widget routing verification and execution (form leads DO route to #dentalmastery-leads via RCV_dm_ghl_events; widget captures still do not); DM handover pack assembly (gated on E1/E2); Denis 5a; a glue-level smoke execution for the optimise verifier, since the extracted-verbatim tests covered the function and the bug sat one line outside it.
...
| 6 | §7: "wmiltd funnel unrehearsed" | False | Portal DB read 2026-09-03: 4 `onboarding_state` rows with `payment_status = paid`; test client `WE BET` created 2026-08-06 12:51 UTC by a live £1 Stripe checkout on app.wmiltd.com. Deletion and refund still owed by the founder. |
