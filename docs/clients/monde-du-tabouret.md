...
**Client wording, Norbert's two-sentence cut, founder-chosen 2026-09-04 (Anthony, first person):** "The one thing still outstanding is on the Shopify side: the Facebook and Instagram sales channel is currently pointed at a Meta pixel outside the Monde du Tabouret business account I manage, and only one of your roughly 1,450 products has reached the catalogue. I need that channel connected to the business account's own pixel and catalogue, with the full range shared, before I can start building the ads."

**Commerce Manager tool gap, founder-directed 2026-09-04.** Norbert briefed Bernard on the gap and Bernard filed a six-item read-tool spec to his own memory (entry a643fffe): (1) business-level pixel edges `owned_pixels` and `client_pixels` with owning business; (2) `owned_product_catalogs` with product_count; (3) catalog detail: external_event_sources, data_sources, diagnostics; (4) feeds and product sets with schedule and last-upload status, to separate "1 product, sync stalled" from "1 product, small store"; (5) the storefront's Shopify web-pixel manifest, the surface that found MDT's pixel; (6) `commerce_merchant_settings` (shops, sales-channel status), unreadable by every token today. Norbert's sequencing for the platform build: 1-3 are the Phase 0 critical path and would have closed today's verification without Code, 4 settles the sync question, 5 and 6 are completeness. Standing rule now on Bernard's record: on Shopify or catalog clients an unreadable surface is a named gap handed to Code for a direct read, never an inference from his own empty tools. The build itself belongs to the platform channel; flagged in PROJECT_STATE §7.
