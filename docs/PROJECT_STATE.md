...
**Sequence from here, revised 2026-08-03 after the numbers were bought.** The agency turned out to be on **LC Phone**, so there is no agency-level "own Twilio" switch. The supported path is a per-location LC Phone disable via GHL's support form (Location ID + Twilio SID + auth token; processed by support, per location, other locations untouched). **Status: forms submitted for two locations, KST (`Zts49PaUrbGfHuBtpknt`) and the WMI operating location; clarifying questions from two separate support agents answered 2026-08-03; awaiting GHL confirmation.** Neither location ever had LC Phone or any number, so there is nothing to release and no porting involved (watch for support calling it "porting" and correct them). When GHL confirms: import +442046529670 into KST (forward to 020 3150 2074, then the one-line swap in the KST site repo `lib/site.ts`) and +442045383367 into WMI for the dialer; verify from the Twilio side that GHL stamped its webhooks onto the numbers, then live test call. Blueprint note: every future client location needs this same one-time support form, now a runbook step. The 07 waits on the mobile bundle (in-review).
