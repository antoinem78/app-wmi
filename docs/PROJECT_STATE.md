...
**Event architecture settled with Steffen's implementing agent, 2026-08-03.** The existing standard Lead event (fires after form submission + DB capture) STAYS; the spec adds downstream, never replaces: call booked as standard Schedule, deposit as Purchase carrying cabin value ($16,900/$25,900), micro-events (email response, FAQ opens, video engagement) as custom events for audience building only, never bid on. Agent adds a conversion_name tag + workshop slug for reporting. Google conversion actions (qualified lead, call booked, deposit) get created in the account build this week; the agent needs their resource IDs (customers/X/conversionActions/Y) before his wiring goes live. Event spec is with the agent; his implementation notes + ETA are the critical path to the 9 August milestone, and M2 (written review + end-to-end test lead in both platforms) gates launch.
