...
**Sequence from here:** bundle approval lands, then founder picks an 020 number from live inventory and confirms purchase, then the mobile bundle is completed and an 07 number bought for SMS, then GHL agency-level Twilio connection (Settings, Phone System, own Twilio) with the numbers in a WMI location so calls and texts run from the LeadConnector app with call recording and history in GHL. After that: the KST tracking number (separate 020, forward to 020 3150 2074, one-line swap in the KST site repo `lib/site.ts`). KST call tracking is founder-green-lit.
