...
// Norbert chat panel: talk to the supervisor before Oscar and Bernard are
// called and dispatched. Talks to /api/norbert/chat. Same streaming NDJSON
// contract and visual language as BernardChat; the conversation persists
// server-side under the "norbert" scope.
...
// Agents hand out download paths and links; make them clickable.
...
const SUGGESTIONS = [
  "What needs my attention across both agents?",
  "What is waiting for my approval right now?",
  "I want to brief Oscar on something. Help me shape it first.",
  "Review the latest staged work before I decide anything.",
];

export function NorbertChat({
  ...
  // Load prior turns on mount. Guarded so a slow hydration response can never
  // clobber a conversation the user has already started.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/norbert/chat");
        if (res.ok) {
  ...
  async function clearChat() {
    ...
    setArtifacts([]);
    try {
      await fetch("/api/norbert/chat", { method: "DELETE" });
    } catch {
      /* best-effort */
    }
  }
  ...
  function addFiles(list: FileList | null) {
    ...
    // the input's value empties it before a deferred setState updater runs.
    const incoming = list ? Array.from(list) : [];
    if (!incoming.length) return;
    const room = Math.max(0, MAX_FILES - pending.length);
    setError(
      incoming.length > room ? `Norbert takes up to ${MAX_FILES} files at a time.` : null,
    );
    if (!room) return;
  ...
  async function send(text: string) {
    ...
    // A file on its own is a valid turn; give Norbert a default instruction.
    ...
    try {
      ...
      if (files.length) {
        ...
        res = await fetch("/api/norbert/chat", { method: "POST", body: form });
      } else {
        res = await fetch("/api/norbert/chat", {
          method: "POST",
  ...
  return (
    <div className={`flex ${heightClass} flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm`}>
      {/* Header */}
      <div className="flex items-center justify-between gap-2 border-b border-zinc-100 bg-gradient-to-r from-[#1F1A3A] to-[#3A2E5C] px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <span className="text-sm font-semibold text-white">Norbert</span>
      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length === 0 && hydrated && (
          <div className="text-sm text-zinc-500">
            <p className="font-medium text-zinc-700">Talk to Norbert first.</p>
            <p className="mt-1 text-xs text-zinc-400">
              I supervise Oscar and Bernard. Discuss the work with me here: I read both
              agents&rsquo; live state, review what they have staged, and on your word I
              dispatch written briefs to either of them. I hold no approval authority;
              every execution gate still answers to you alone.
            </p>
        ...
        <div className="flex flex-wrap gap-2 border-t border-zinc-100 px-3 py-2">
          {artifacts.map((a) => (
            <a
              key={a.href}
              href={a.href}
              className="inline-flex items-center gap-1.5 rounded-full border border-zinc-300 bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-700 hover:border-[#1F1A3A] hover:text-[#1F1A3A]"
            >
              <DocIcon />
              {a.label}
            </a>
      ...
      <form
        ...
        className={`border-t p-3 ${dragging ? "border-[#1F1A3A] bg-zinc-50" : "border-zinc-100"}`}
      >
        {pending.length > 0 && (
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-lg border border-zinc-300 px-2.5 py-2 text-zinc-600 hover:border-[#1F1A3A] hover:text-[#1F1A3A] disabled:opacity-40"
          >
            <ClipIcon />
          </button>
          <textarea
            ...
            rows={1}
            placeholder={pending.length ? "Ask Norbert about the attached…" : "Discuss it with Norbert…"}
            className="max-h-32 flex-1 resize-none rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:border-[#1F1A3A] focus:outline-none"
          />
          <button
            type="submit"
            disabled={loading || (!input.trim() && pending.length === 0)}
            className="shrink-0 rounded-lg bg-[#1F1A3A] px-4 py-2 text-sm font-medium text-white disabled:opacity-40"
          >
            Send
          </button>
        </div>
